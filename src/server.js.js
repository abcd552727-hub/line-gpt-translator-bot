...
function createPlan(plans, userId, planType, groupLimit, days) {
  ensurePlan(plans, userId);
  plans[userId].planType = planType;
  plans[userId].groupLimit = groupLimit;
  plans[userId].vipExpiresAt = addDays(days);
  plans[userId].boundGroups = [];
}

function disablePlan(plans, userId) {
  ensurePlan(plans, userId);
  plans[userId].vipExpiresAt = new Date(Date.now() - 1000).toISOString();
}
...
async function handlePostback(event) {
  ...
  if (!isSuperAdmin(userId) && !canManageGroup(groups, plans, chatId, userId)) {
...
async function handleCommand(event, rawText) {
  ...
  if (cmd === "/menu") {
    if (!superAdmin && !canManageGroup(groups, plans, chatId, userId)) {
  ...
  if (cmd === "/開通1群") {
    ...
    createPlan(plans, arg, "limited_groups", 1, 30);
    savePlans(plans);

    await replyText(
      event.replyToken,
      `已開通 1群 / 30天\n使用者：${arg}\n到期：${formatDateTime(plans[arg].vipExpiresAt)}`
    );
    return true;
  }
  ...
  if (cmd === "/開通3群") {
    ...
    createPlan(plans, arg, "limited_groups", 3, 30);
    savePlans(plans);
  ...
  if (cmd === "/開通5群") {
    ...
    createPlan(plans, arg, "limited_groups", 5, 30);
    savePlans(plans);
  ...
  if (cmd === "/開通不限30") {
    ...
    createPlan(plans, arg, "unlimited_groups", null, 30);
    savePlans(plans);

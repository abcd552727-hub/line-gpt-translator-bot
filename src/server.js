...
function canUseGroup(plan, groupId) {
  ...
  if (plan.plan_type === "free_trial" || plan.plan_type === "trial_7days") {
...
async function handleFollow(event) {
  ...
  if (!plan) {
    const trialPlan = createFreeTrialPlanObject(userId);
    await savePlan(trialPlan);
  }

  await replyMessages(event.replyToken, [
    buildLanguageMenuFlex(),
    { type: "text", text: "歡迎使用翻譯機器人。你目前有基本試用，每天有次數限制。" },
  ]);
}
...
async function handleCommand(event, rawText) {
  ...
  if (cmd === "/trial3") {
    ...
    const nextPlan = create7DayTrialPlanObject(ownerId);
    nextPlan.vip_expires_at = addDays(3);
    nextPlan.trial_type = "3天試用";
...
async function handleTextMessage(event) {
  ...
  if (actingPlan.daily_limit) {
    const limitResult = await checkDailyLimit(
      chatType === "user" ? userId : actingPlan.user_id,
      chatId,

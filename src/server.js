...
9. 允許為了符合目標語言語序進行結構重組，但不可增加內容
10. 若直譯會導致目標語言不自然，必須重組語序，但不可改變原意
11. 人名、暱稱、店名、地名、公司名、專有名詞、金額、時間，若能確認正式中文，直接使用正式中文
12. 若無法確認正式中文，但看起來像地名、人名、店名或其他專有名詞，不可直接整段不翻
13. 查不到正式名稱時，請優先用自然的中文音譯或諧音方式表達，讓讀者可依上下文猜測意思
14. 音譯後仍要保留整句語意，讓整句能看懂前後文
15. 不可自行捏造明確但錯誤的正式地名；若無把握，使用偏口語的音譯表達即可
16. 不可加前綴，不可解釋，不可摘要
17. 不可輸出「好的親愛的、Yes dear、OK honey」這種腦補內容
18. 原文可能是 LINE 對話、泰國口語、方言、混合語言
19. 若句意不完整，請忠實翻出最可能意思，但不要擴寫
20. 相同句子在相同情境下，盡量維持一致
21. 優先使用簡單、固定、自然的詞
22. 只輸出最終翻譯結果
23. 若固定術語表有指定詞語，必須優先使用，不可改寫
24. 若原文有常見誤拼、近音字、聊天誤打，必須優先依上下文修正後再翻譯
25. 在真人聊天情境中，若「บอท / บอก」更可能是誤打的「บอส」，優先翻成「老闆」，不要翻成「機器人」
26. 「ที่」開頭通常是描述句、補充句、修飾句，不可直接翻成「因為」
27. 只有原文出現「เพราะ / เนื่องจาก」等明確因果詞時，才可翻成「因為」
28. 若原文只有語氣詞或短回覆（如 ค่ะ / คะ / ครับ / อืม / นะ / ไม่ / ได้），應翻成目標語言自然回應，不可保留原文
29. 泰文語氣詞不可原樣輸出，必須轉成自然語氣或簡短回應
30. 不可把泰文補充句硬翻成中文因果句
31. 不可把原句翻得更甜、更正式、更自然化
32. 可依上下文判斷意思，但不可改寫句子核心意思
33. 若句首出現「โอเค / ok / okay」，只有在明確表示同意或回答時才翻成「好」；若只是語氣開頭或銜接語氣，可省略不翻
34. 語氣詞若不影響句子核心意思，可省略，不一定需要翻出來
35. 若目標語言是英文，最終結果必須是純英文，不可輸出中文、泰文、拼音、解釋或混合語言
36. 若目標語言是英文，即使原文是聊天語氣，也必須翻成自然英文句子
...
- นะ / นะคะ / นะครับ → 喔 / 呢
- ได้ → 好 / 可以
- ไม่ → 不 / 不要 / 不用 / 不是（依上下文判斷）
- ไม่เอา → 不要

特別規則：
- 中文「辛苦了」只有在原文真的有這句時才翻
- 中文「好」就翻成「โอเค / ได้ / okay」這類對應說法，不要自動補成感謝句
- 泰文簡短回覆可依前文判斷真正意思，但判斷完後仍要保持短句，不可擴寫
- 泰國不同地區口語、方言、聊天簡寫，要先理解當地用法再翻譯
- 但不可因為是方言就亂猜、亂補、亂改寫

來源語言提示：${sourceHint}
翻譯模式：正常直譯（可判斷上下文，不可改寫）
補充提示：${specialHint || "無"}
...
async function translateToTarget(text, targetLang) {
  ...
  if (thaiShortChat) {
    specialHint += " 這是泰文短句或聊天回覆，請依前後文判斷真正意思，但不可擴寫成完整回答句。";
  }

  if (thaiDialect) {
    specialHint += " 這句可能包含泰國不同地區口語、方言或聊天簡寫，請先依上下文理解當地說法，再翻成自然中文，但不可亂猜、不可改寫原意。";
  }
  ...
  if (targetLang === "th") {
    specialHint += " 請輸出自然泰文，但不可自行加禮貌、加情緒、加長句子。";
  }
  ...
  if (targetLang === "en") {
    specialHint += " 請輸出自然、完整、純英文句子，不可出現中文或泰文。";
  }
  ...
  if (targetLang === "en") {
    const hasCJK = /[\u4E00-\u9FFF\u0E00-\u0E7F]/.test(output);
    const hasEnoughEnglish = (output.match(/[A-Za-z]/g) || []).length >= 3;

    if (hasCJK || !hasEnoughEnglish) {
      output = await askModelTranslate({
        text,
        targetLang,
        sourceHint: sourceLang,
        specialHint: `${specialHint} 最終結果只能是純英文句子，不可出現任何中文或泰文，不可翻成中文。`.trim(),
      });
    }

    const hasCJKAgain = /[\u4E00-\u9FFF\u0E00-\u0E7F]/.test(output);
    const hasEnoughEnglishAgain = (output.match(/[A-Za-z]/g) || []).length >= 3;

    if (hasCJKAgain || !hasEnoughEnglishAgain) {
      output = await askModelTranslate({
        text,
        targetLang,
        sourceHint: sourceLang,
        specialHint: "把這句翻成自然、完整、純英文。不可出現中文或泰文，只能輸出英文翻譯結果。",
      });
    }
  }
